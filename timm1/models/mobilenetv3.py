...
class MobileNetV3(nn.Module):
    ...
    def __init__(self, block_args, num_classes=1000, in_chans=3, stem_size=16, num_features=1280, head_bias=True,
                 ...
                 round_chs_fn=round_channels, drop_rate=0., drop_path_rate=0., global_pool='avg',args=None):
        ...
        if args!=None and args.model=='SLaK_tiny' and ('FD' in args.distill_type):
            #768,14,14/7,7
            self.project = nn.Sequential(
                nn.Conv1d(
                in_channels=960,
                out_channels=998, kernel_size=1),
            )
    ...
    def reset_classifier(self, num_classes, global_pool='avg'):
        ...
        self.global_pool = SelectAdaptivePool2d(pool_type=global_pool)
        self.classifier = Linear(self.num_features, num_classes) if num_classes > 0 else nn.Identity()
    def project_slak(self):
        x=self.feature.reshape((self.feature.shape[0],self.feature.shape[1],-1))
        x=self.project(x)
