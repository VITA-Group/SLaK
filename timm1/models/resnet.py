...
vit_dict={3:768}
swin_resize_dict={0:7,1:7,2:14,3:28}
vit_resize_dict={0:14}
class ResNet(nn.Module):
    ...
    def __init__(self, block, layers, num_classes=1000, in_chans=3,
                 ...
                 drop_block_rate=0., global_pool='avg', zero_init_last_bn=True, block_args=None,args=None):
        ...
        if zero_init_last_bn:
            for m in self.modules():
                if hasattr(m, 'zero_init_last_bn'):
                    m.zero_init_last_bn()
        if args!=None  and ('LayerFeature' in args.distill_type):
            self.projects=nn.ModuleList()
            for i in range(self.args.feature_n):
                j=3-i
                if args.model=='SLaK_tiny':
                    if resnet_dict[j]!=slak_dict[j]:
                        self.projects.append(nn.Sequential(nn.Conv1d(in_channels=resnet_dict[j],out_channels=slak_dict[j], kernel_size=1),))
                    else:
                        self.projects.append(None)
                elif args.model=='swin':
                    if resnet_dict[j]!=swin_dict[j]:
                        self.projects.append(nn.Sequential(nn.Conv1d(in_channels=resnet_dict[j],out_channels=swin_dict[j], kernel_size=1),))
                    else:
                        self.projects.append(None)
                elif args.model=='convnext':
                    if resnet_dict[j]!=convnext_dict[j]:
                        self.projects.append(nn.Sequential(nn.Conv1d(in_channels=resnet_dict[j],out_channels=convnext_dict[j], kernel_size=1),))
                    else:
                        self.projects.append(None)
                elif args.model=='vit':
                    if resnet_dict[j]!=vit_dict[j]:
                        self.projects.append(nn.Sequential(nn.Conv1d(in_channels=resnet_dict[j],out_channels=vit_dict[j], kernel_size=1),))
                    else:
                        self.projects.append(None)
                else:
                    assert False
        if args!=None and ("FD") in args.distill_type:
            if args.model=='efficientnet':
                self.projector=nn.Linear(2048,1536)
    ...
    def project(self):
        ...
        for i in range(self.args.feature_n):
            ...
            if self.projects[i]!=None:
                b,n,h,w=self.stage_features[j].shape
                out.append(self.projects[i](self.stage_features[j].view(b,n,-1)).view(b,-1,h,w))
            else:
                out.append(self.stage_features[j])
        if self.args.model=='swin':
            for i in range(self.args.feature_n):
                if i>0:
                    out[i]=F.interpolate(out[i],size=(swin_resize_dict[i]))
        if self.args.model=='vit':
            for i in range(self.args.feature_n):
                out[i]=F.interpolate(out[i],size=(vit_resize_dict[i]))
        return out        
    def project_rebutal(self):
        if self.args.model=='resnet50d':
            return self.feature
        elif self.args.model=='efficientnet':
            return self.projector(self.feature)
       
    def project_vit(self):
        pass
    ...
    def forward_features(self, x):
        ...
        x = self.maxpool(x)
        x = self.layer1(x)
        self.stage_features[0]=x.clone()
        x = self.layer2(x)
        self.stage_features[1]=x.clone()
        x = self.layer3(x)
        self.stage_features[2]=x.clone()
        x = self.layer4(x)
        self.stage_features[3]=x.clone()
        return x
